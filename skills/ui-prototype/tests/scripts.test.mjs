...
test("skill defines focused, comparison, and measurable layout contracts", async () => {
  ...
  assert.match(skillSource, /computed\s+`width`\s+and\s+`max-width`/)
  assert.match(skillSource, /open and inspect every\s+named surface once/)
  assert.match(skillSource, /exactly one open-state screenshot/)
...
test("skill keeps theme contextual and the visual system restrained", async () => {
  ...
  assert.match(skillSource, /a decorative wall of Cards/)
  assert.match(skillSource, /equal\s+containers or Cards are appropriate/)
  assert.match(skillSource, /Base UI `data-\*` selectors can outrank/)
  assert.match(skillSource, /never excuses clipped\s+headings/)
